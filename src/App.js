...
class App extends Component {
	...
	render() {
		...
		const columns = [
			...
			{ title: 'Done', items: items.filter( item => item.status === 'Done'), icon: <CheckCircleIcon />},
				...
				<div className="App">
					...
					<div className="enable-remove-mode">
						<If test={!this.state.removeMode}>
							<Fab onClick={this.enableRemoveMode}>
								<EditIcon />
							</Fab>
						</If>
						<If test={this.state.removeMode}>
							<Fab secondary={true} onClick={this.disableRemoveMode}>
								<CloseIcon />
							</Fab>
						</If>
					</div>
